export class GameDialog {
  ...
  typeText() {
    if (this.currentTextIndex < this.dialogSections[this.currentSectionIndex].length) {
      this.dialogText.textContent += this.dialogSections[this.currentSectionIndex].charAt(this.currentTextIndex);
      this.currentTextIndex++;
      // this.typeText();
      setTimeout(() => this.typeText(), 50);
    } else {
      this.continueText.classList.remove("hidden");
      document.addEventListener("keydown", (event) => this.handleKeyPress(event) , {once: true});
    }
}
